...
A constructor can also take no arguments, in which case we can simply use an empty list to represent that, but can also take an argument of the same type it is trying to construct (like the \mintinline{haskell}{S} constructor from the previous section). 
...
These parameters are needed for the remaining constructors: the \mintinline{haskell}{Scns} constructor corresponds to the  case where the constructor is left untouched but some of the arguments have changed.
For this reason its second argument consists of the predicate \mintinline{haskell}{at} applied to the list of arguments which describes how to transform them. 
...
These views are not completely equal, but the choice of constructor is, this makes sense as we are transforming a \mintinline{haskell}{Node} into another one. The spine produced
...
The other case is covered by the \mintinline{haskell}{As} constructor, recall that in this case \mintinline{haskell}{Diagonal} simply lifts \mintinline{haskell}{Usingl} to a pair of elements of type \emph{u}, so the first parameter can be read as: a pair of \mintinline{haskell}{Usingl u} where the first element is the source and the other is the destination.
...
We finally have to define a function that computes the diff in terms of \mintinline{haskell}{Almu}s. This will call \mintinline{haskell}{diffS} in case of two matching constructors from which we can compute the spine wrapping that with the corresponding \mintinline{haskell}{Alspn} constructor. In the other cases it will attempt the insertion (resp. deletion) at the constructor level by recording the constructor being inserted (deleted) and producing a \mintinline{haskell}{Ctx} which describes where the original tree is attached in respect to the added (deleted) constructor.
...
\begin{itemize}
\item One is to explore the possibility of using the standard unix \mintinline{haskell}{diff3} algorithm as an oracle to prune the alignment trees that are being generated. 
...
We will call this datatype \mintinline{haskell}{DiffAction} and it will have the following definition.
...
However, to perform the optimal patch between these two programs we clearly want to insert a \mintinline{haskell}{Seq} on the first line and proceed calculating the diff from there, but our \toHaskell{DiffAction} does not contain any insertion there. 
...
\begin{minted}[fontsize=\footnotesize, linenos]{clojure}
...
In this case, according to \mintinline{haskell}{diff3} we are supposed to copy lines 1 and 3 and modify line 2. The problem is that -- in the source file -- we have a \toHaskell{Cons} node 
...
\centering
\includegraphics[scale=0.6]{shortcomings-src.png}
\end{minipage}
\begin{minipage}{0.4\linewidth}
\centering
\includegraphics[scale=0.6]{shortcomings-dst.png}
\end{minipage}
\vspace{.6em}
...
\begin{minipage}{0.4\linewidth}
\centering
\includegraphics[scale=0.6]{shortcomings-solved-src.png}
\end{minipage}
\begin{minipage}{0.4\linewidth}
\centering
\includegraphics[scale=0.6]{shortcomings-solved-dst.png}
\end{minipage}
\vspace{.6em}
